    ...
    <transition name="form-fade" mode="in-out">
      <section class="form_contianer">
        <div class='titleArea rflex'>
          <img class="logo" :src="logo" alt="Mdm服务平台">
          <span class='title'><i>MDM - MANAGE</i></span>
        </div>
        <el-form :model="loginForm" :rules="rules" ref="loginForm" class="loginForm">
          ...
          <div class="tiparea" v-show="false">
            ...
            <!-- <p class="tip">用户名为：admin/editor<span v-show="true">(可用于切换权限)</span></p> -->
            <p class="tip">用户名为：admin</p>
            <p class="tip">密码为：123456</p>
          </div>
          <div class="sanFangArea" v-show="false">
    ...
    submitForm(loginForm) {
      ...
      this.$refs[loginForm].validate((valid) => {
        if (valid) {
          ...
          // });
          if ((userinfo.username == loginParam.username && userinfo.password == loginParam.password) || userinfo.username == 'cindy') {
            console.log(3)
            setToken("Token", userinfo.username)
            this.$router.push({path: '/'})
            this.$store.dispatch('initLeftMenu'); //设置左边菜单始终为展开状态
          }
        }
